...
export class ProductsService {
  ...
  async create(creatorId: string, data: Record<string, unknown>): Promise<unknown> {
    const rows = await this.db.query(
      `INSERT INTO products (
         ...
         delivery_type, escrow_required, service_slots, event_date, event_time, variants,
         chain, is_wholesale, token_gated, category, categories, tags, go_live_at
       ) VALUES (
      ...
      [
        ...
        data.cover_uri ?? null,
        data.price_tiers ? JSON.stringify(data.price_tiers) : null,
        data.interval ?? null,
        data.pinned ?? false,
        ...
        data.service_slots ? JSON.stringify(data.service_slots) : null,
  ...
  async update(productId: string, creatorId: string, data: Record<string, unknown>): Promise<unknown> {
    ...
    const rows = await this.db.query(
      ...
      [data.title ?? null, data.description ?? null, data.price ?? null, productId]
    );
    return rows[0];
  }
  ...
  async getRelated(productId: string, limit = 6): Promise<unknown[]> {
    const rows = await this.db.query(
      `SELECT creator_id, type FROM products WHERE id = $1`,
      [productId]
    );
    if (!rows[0]) return [];
    const { creator_id, type: product_type } = rows[0];
    return this.db.query(
