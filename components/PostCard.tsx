...
export function PostCard({
  ...
  const fetchComments = useCallback(async (skipLoading = false) => {
    ...
    const data = await apiGet<CommentRow[]>(`/posts/${post.id}/comments`).catch(() => null);
  ...
  const submitComment = useCallback(async () => {
    ...
    const payload: { body: string; parentId?: string } = { body: trimmed };
    if (replyingToCommentId) payload.parentId = replyingToCommentId;
    await apiPost(`/posts/${post.id}/comments`, payload).catch(() => {});
    ...
    async (commentId: string) => {
        ...
        prev.map((x) =>
          x.id === commentId
            ? {
                ...x,
                is_liked: nextLiked,
                is_disliked: nextLiked ? false : x.is_disliked,
                like_count: Math.max(0, x.like_count + (nextLiked ? 1 : -1)),
                dislike_count: nextLiked && x.is_disliked ? Math.max(0, x.dislike_count - 1) : x.dislike_count,
              }
            : x
        )
      );
      const result = await apiPost<{ liked: boolean; likeCount: number; dislikeCount: number }>(
      ...
      if (result) {
        setComments((prev) =>
          prev.map((x) =>
            x.id === commentId
              ? { ...x, is_liked: result.liked, like_count: result.likeCount, dislike_count: result.dislikeCount }
              : x
          )
        );
      }
    },
    [user?.id, comments, post.id]
  );
    ...
    async (commentId: string) => {
        ...
        prev.map((x) =>
          x.id === commentId
            ? {
                ...
                is_liked: nextDisliked ? false : x.is_liked,
                dislike_count: Math.max(0, x.dislike_count + (nextDisliked ? 1 : -1)),
                like_count: nextDisliked && x.is_liked ? Math.max(0, x.like_count - 1) : x.like_count,
              }
            : x
        )
      );
      const result = await apiPost<{ disliked: boolean; likeCount: number; dislikeCount: number }>(
      ...
      if (result) {
        setComments((prev) =>
          prev.map((x) =>
            x.id === commentId
              ? { ...x, is_disliked: result.disliked, like_count: result.likeCount, dislike_count: result.dislikeCount }
              : x
          )
        );
      }
